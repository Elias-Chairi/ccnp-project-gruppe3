...
func TestEncode_RegisterControlMessage(t *testing.T) {
	...
	expectedTLV, _ := tlv.NewTLV(uint8(constants.REGISTER_CONTROL), []byte{})
...
func TestDecode_RegisterControlMessage(t *testing.T) {
	...
	registerControlTLV, _ := tlv.NewTLV(uint8(constants.REGISTER_CONTROL), []byte{})
...
func TestDecodeRegisterControlMessage_InvalidArgument(t *testing.T) {
	...
	// non-empty value
	nonEmptyTLV, _ := tlv.NewTLV(uint8(constants.REGISTER_CONTROL), []byte{0x01, 0x02})
